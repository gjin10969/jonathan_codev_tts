...
def auto_train(data_path, save_path='model.pth', load_model: str | None = None, save_epochs=1):
    ...
    if load_model and os.path.isfile(load_model):
        ...
        model_training = CustomTokenizer.load_from_checkpoint(load_model, 'cpu')
    else:
        print('Creating new model.')
        model_training = CustomTokenizer(version=1).to('cpu')  # Settings for the model to run without lstm
    ...
    while 1:
        for i in range(save_epochs):
            ...
            for x, y in zip(data_x, data_y):
                model_training.train_step(torch.tensor(x).to('cpu'), torch.tensor(y).to('cpu'), j % 50 == 0)  # Print loss every 50 steps
                j += 1
        save_p = save_path
